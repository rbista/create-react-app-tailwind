...
const App = (props) => {
  ...
  const handleSaveNewAction = (event) => {
    if(newAction.name && newAction.value){
     console.log("EVENT:",newAction, event);
    ...
    <div className="bg-gray-900 p-6 h-screen grid grid-cols-5">
      ...
      <div className={`h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-opacity-90 bg-gray-900 ${modalStatus ? 'block' : 'hidden'}`}>
        <dialog open={modalStatus} className="rounded-lg bg-white rounded shadow-lg w-10/12 md:w-1/2 p-0">
          ...
          {/* <!-- modal body --> */}
          <div className="p-3">
            <form id="actionForm" onSubmit={()=> false}>
              <label className="block">
                <span className="text-gray-700">Action Name</span>
                <input onChange={(event) => handleActionForm(event.targetvalue, newAction.value)} className="block w-full appearance-none placeholder-gray-500 border border-gray-400 rounded-md py-3 px-4 my-1 text-gray-700 leading-5 focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="Action Name" required />
              </label>
              <label className="block">
