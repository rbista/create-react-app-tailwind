  ...
  return (
    <div className="bg-gray-900 p-6 h-screen grid grid-cols-5">
      ...
      <button className="p-4 bg-green-600 rounded-lg font-bold text-white mt-5 hover:bg-gray-600">
        ...
        {gameStatus && (<button className="p-4 w-full bg-red-600 rounded-lg font-bold text-white mt-5 hover:bg-gray-600" onClick={handleStopGame}>
          Stop Game
        </button>)}
      ...
      <div className={`sm:col-span-3 md:col-span-4 mx-auto flex flex-col justify-center content-center place-content-center ${showTimer ? 'block' : 'hidden'}`}>
        <h1 className={`text-${fontSize} text-white ${fontCapital ? 'uppercase' : 'lowercase'}`}>{countdown}</h1>
         <h2 className={`text-${fontSize} text-white ${fontCapital ? 'uppercase' : 'lowercase'}`}>Get Ready</h2>
         </div>
