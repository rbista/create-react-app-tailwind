...
const App = (props) => {
  ...
  return (
    <div className="bg-gray-900 p-6 h-screen grid grid-cols-5">
      ...
      {showActions && (<div className="sm:col-span-3 md:col-span-4 text-white grid grid-cols-3"> {ACTIONS.map((act, index)=>(<div className=" col-span-1 rounded-lg bg-gray-700 p-3 m-3 h-auto" key={index+act.value}>L</div>))}</div>)}
    </div>
