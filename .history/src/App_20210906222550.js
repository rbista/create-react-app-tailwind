    ...
    <div className="bg-gray-900 p-6 h-screen grid grid-cols-5">
      ...
      {showActions &&
        (<div className="sm:col-span-3 md:col-span-4 text-white p-6">
          <h2 className="text-lg text-gray-100 ">Action Lists:</h2>
      ...
      <div className={`h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-opacity-90 bg-gray-900 ${modalStatus ? 'block' : 'hidden'}`}>
        <dialog open={modalStatus} className="rounded-lg bg-white rounded shadow-lg w-10/12 md:w-1/2 p-0">
          ...
          <div class="border-b px-4 py-4 flex justify-between items-center">
            ...
            <button class="text-black close-modal" onClick={() => handleModalChange('close')}><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="p-3">
            ...
            <label class="block">
              ...
              <input value={newAction.name} onChange={(event)=>handleActionForm(event.targetvalue,newAction.value)} class="block w-full appearance-none placeholder-gray-500 border border-gray-400 rounded-md py-3 px-4 my-1 text-gray-700 leading-5 focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="Action Name" required />
            </label>
            <label class="block">
              <span class="text-gray-700">Action Letter</span>
              <input value={newAction.value} onChange={(event)=>handleActionForm(newAction.name,event.target.value)} class="block w-full appearance-none placeholder-gray-500 border border-gray-400 rounded-md py-3 px-4 my-1 text-gray-700 leading-5 focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="Action Letter" required/>
            </label>
            </form>
          </div>
          <div class="flex justify-end items-center w-100 border-t p-3">
            <button class="bg-white hover:bg-gray-300 px-3 py-1 rounded mr-1 close-modal" onClick={() => handleModalChange('close')}>Cancel</button>
            <button class="bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded text-white" formTarget="actionForm" type="submit" onClick={handleSaveNewAction} >Save</button>
            {/* </div> */}
            {/* </div> */}
