...
const App = (props) => {
  ...
  // const setFontSize=()=
  return (
    <div className="bg-gray-900 min-h-screen p-3 grid md:grid-cols-5 gap-2">

      <div className="col-start-1 sm:col-span-1 rounded-lg">
      ...
      {displayContent === START && (<div className="sm:col-span-4  flex flex-col flex-wrap justify-center content-center">
        ...
        <div className={`grid grid-cols-12 ${showContent ? 'block' : 'hidden'}`}>
          <h1 className={`col-span-12 text-${fontSize} text-white ${fontCapital ? 'uppercase' : 'lowercase'}`}>{displayChar.name}<span className={`text-2xl text-green-600 font-bold`}>{displayChar.value}</span></h1>
        </div>
      ...
      {displayContent === ACTIONLIST &&
        (<div className="col-span-1 sm:col-span-4 text-white p-6">
