...
const Game = (props) => {
  ...
  const { gameDuration, alphaRules, timerSpeed,setTimerSpeed, fontSize, fontCapital,setFontCapital, gameStatus, setGameStatus, defaultData } = props;
  ...
  return (
    <div className=" flex flex-col flex-wrap justify-center content-center h-full">
      {!showTimer && !showContent && (
        <Settings defaultData={defaultData} timerSpeed={timerSpeed} setTimerSpeed={setTimerSpeed} fontCapital={fontCapital} setFontCapital={setFontCapital} gameStatus={gameStatus} setNoOfChars={setNoOfChars} />
      )}
      {showContent && (
