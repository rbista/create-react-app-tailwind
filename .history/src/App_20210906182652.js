...
const App = (props) => {
  ...
  // const setFontSize=()=
  return (
    <div className="bg-gray-900 p-6 h-full grid grid-cols-5">
      ...
      <button className="p-4 bg-green-600 rounded-lg font-bold text-white mt-5 hover:bg-gray-600">
        ...
        <button className="p-4 bg-green-600 rounded-lg font-bold text-white mt-5 hover:bg-gray-600 w-full" onClick={handleStartGame}>
          Start Game
        </button>
      </div>
      <div className={`sm:col-span-3 md:col-span-4 flex flex-wrap justify-center content-center ${showContent?'hidden':'block'}`}>
      <h1 className={`text-${fontSize} text-white ${fontCapital ? 'uppercase': 'lowercase'}`}>{displaChar}</h1>
      </div>
      <div id="countdown" className={`sm:col-span-3 md:col-span-4 flex flex-wrap justify-center content-center ${showTimer?'block':'hidden'}`}>
