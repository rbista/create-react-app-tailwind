      ...
      <div className={`h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-opacity-90 bg-gray-900 ${modalStatus ? 'block' : 'hidden'}`}>
        <dialog open={modalStatus} className="rounded-lg bg-white rounded shadow-lg w-10/12 md:w-1/2 p-0">
          ...
          <div className="p-3">
            <form id="actionForm" action="/">
              <label className="block">
                ...
                <input value={newAction.name} onChange={(event) => handleActionForm(event.targetvalue, newAction.value)} className="block w-full appearance-none placeholder-gray-500 border border-gray-400 rounded-md py-3 px-4 my-1 text-gray-700 leading-5 focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="Action Name" required />
              </label>
              <label className="block">
                <span className="text-gray-700">Action Letter</span>
                <input value={newAction.value} onChange={(event) => handleActionForm(newAction.name, event.target.value)} className="block w-full appearance-none placeholder-gray-500 border border-gray-400 rounded-md py-3 px-4 my-1 text-gray-700 leading-5 focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="Action Letter" required />
              </label>
            </form>
          </div>
          <div className="flex justify-end items-center w-100 border-t p-3">
            ...
            <button className="bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded text-white" form="actionForm" type="submit" onClick={handleSaveNewAction} >Save</button>
            {/* </div> */}
            {/* </div> */}
