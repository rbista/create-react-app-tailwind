...
const Rules = (props) => {
  ...
  const { fontCapital, gestures,alphaRules, setAlphaRules, defaultData } = props;
  ...
  useEffect(() => {
    console.log("IF::::::::::::::",Object.keys(defaultData).length);
    if(!Object.keys(defaultData).length){
      setAlphaRules(genCharAction());
    }
  }, []);
  ...
  const genCharAction = () => {
    ...
    ALPHABET.map(alpha => {
     ...
     console.log(" Generating rules:",tempRules );
