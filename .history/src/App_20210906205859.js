...
const App = (props) => {
  ...
  const genCharAction = () => {
    let tempRules = [];
    ALPHABET.map(alpha => {
       console.log("RAND akjsdf: ", flex flex-wrap)
        const tempVaue=randomAction();
      tempRules = [...tempRules, { name: alpha, value: ACTIONS[tempVaue].value }];
    })
  ...
  return (
    <div className="bg-gray-900 p-6 h-screen grid grid-cols-5">
      ...
      <div  className="col-span-full flex flex-wrap justify-center content-center">
        <button className="p-4 bg-green-600 rounded-lg font-bold text-white hover:bg-green-700 w-max" onClick={genCharAction}>
            Generate New Rules
          </button></div>
