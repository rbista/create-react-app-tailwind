...
const App = (props) => {
  ...
  const randomAction = (max = ACTIONS.length) => Math.floor(Math.random() * (max - 1));
  ...
  useEffect(() => {
    ...
    if (noOfChars > 0 && showContent) {
      charInterval = window.setInterval(() => {
        setDisplayChar(ALPHABET[randomChar()]);
  ...
  return (
    <div className="bg-gray-900 p-6 h-screen grid grid-cols-5">
      ...
      <button className="p-4 bg-green-600 rounded-lg font-bold text-white mt-5 hover:bg-gray-600">
        ...
        <button className="p-4 bg-green-600 rounded-lg font-bold text-white mt-5 hover:bg-green-700 flex items-center">
      ...
      {!showRules &&(<div className="sm:col-span-3 md:col-span-4 ">
      <div className={`flex flex-wrap justify-center content-center ${showContent ? 'block' : 'hidden'}`}>
        <h1 className={`text-${fontSize} text-white ${fontCapital ? 'uppercase' : 'lowercase'}`}>{displaChar}<span className={`text-2xl text-green-600 font-bold`}>L</span></h1>
      </div>
      <div className={`mx-auto flex flex-col justify-center content-center text-center ${showTimer ? 'block' : 'hidden'}`}>
        <h1 className={`text-${fontSize} text-white `}>{countdown}</h1>
        <h2 className="text-lg text-gray-100">Get Ready</h2>
      </div>
      <div id="countdown" className={`flex flex-wrap justify-center content-center ${!showTimer && !showContent ? 'block' : 'hidden'}`}>
      ...
      {showRules && (<div className="grid grid-cols-7 sm:col-span-3 md:col-span-4 text-center">
        {ALPHABET.map((alpha)=>(<div className="col-span-1 bg-gray-600 m-2 flex flex-wrap justify-center content-center rounded-lg">
      <p className={`text-white text-6xl  ${fontCapital ? 'uppercase' : 'lowercase'}`}>
         { alpha}  <span className={`text-2xl text-green-600 font-bold`}>{ACTIONS[randomAction()]}</span></p>
        </div>))}
