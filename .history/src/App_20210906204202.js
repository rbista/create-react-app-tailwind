...
const App = (props) => {
  ...
  const [displaChar, setDisplayChar] = useState([]);
  ...
  useEffect(() => {
    ...
    if (noOfChars > 0 && showContent) {
      charInterval = window.setInterval(() => {
        setDisplayChar(setAlphaRules[randomChar()]);
    ...
    <div className="bg-gray-900 p-6 h-screen grid grid-cols-5">
      ...
      {!showRules && (<div className="sm:col-span-3 md:col-span-4 flex flex-wrap justify-center content-center">
        <div className={` ${showContent ? 'block' : 'hidden'}`}>
          <h1 className={`text-${fontSize} text-white ${fontCapital ? 'uppercase' : 'lowercase'}`}>{displaChar.name}<span className={`text-2xl text-green-600 font-bold`}>{displaChar.value}</span></h1>
        </div>
