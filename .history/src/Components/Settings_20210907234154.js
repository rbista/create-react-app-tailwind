...
const Settings = (props) => {
  ...
  const {gameStatus, setNoOfChars, defaultData}=props;
  const fontSizeLength = FONTSIZEARRAY.length;


  const [defaultData={defaultData}, setFromLocalStorage] = useState({});

  const tempSlider = defaultData={defaultData}.fontSize ? FONTSIZEARRAY.indexOf(defaultData={defaultData}.fontSize) : fontSizeLength - 1;

  const [sliderValue, setSliderValue] = useState(tempSlider);
  const tempFontSize = defaultData={defaultData}.fontSize ? defaultData={defaultData}.fontSize : FONTSIZEARRAY[fontSizeLength - 1];
  console.log(".--------------", tempFontSize);

  const [fontSize, setFontSize] = useState(tempFontSize);
  const tempTimer = defaultData={defaultData}.timerSpeed ? defaultData={defaultData}.timerSpeed : 1;
  const [timerSpeed, setTimerSpeed] = useState(tempTimer);

  const tempCap = defaultData={defaultData}.fontCapital ? defaultData={defaultData}.fontCapital : true;
  const [fontCapital, setFontCapital] = useState(tempCap);
  const tempDur = defaultData={defaultData}.gameDuration ? defaultData={defaultData}.gameDuration : 10;
  ...
  const handleFontChange = (event, b) => {
    ...
    localStorage.setItem('mind-game', JSON.stringify({ ...defaultData={defaultData}, fontSize: FONTSIZEARRAY[value] }));
  }

  const handleTimerSpeedChange = (event, b) => {
    const { value } = event.target;
    setTimerSpeed(value);
    localStorage.setItem('mind-game', JSON.stringify({ ...defaultData={defaultData}, timerSpeed: value }));
  }
  ...
  const handleDurationChange = (event, b) => {
    ...
    localStorage.setItem('mind-game', JSON.stringify({ ...defaultData={defaultData}, gameDuration: value, noOfChars: parseInt(value / timerSpeed) }));
  }

  const handleFontCapChange = () => {
    setFontCapital(!fontCapital);
    localStorage.setItem('mind-game', JSON.stringify({ ...defaultData={defaultData}, fontCapital: !fontCapital }));
  }
