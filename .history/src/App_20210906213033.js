  ...
  return (
    <div className="bg-gray-900 p-6 h-screen grid grid-cols-5">
      ...
      {showActions && (<div className="sm:col-span-3 md:col-span-4 text-white"><div className="grid grid-cols-3"> 
        {ACTIONS.map((act, index)=>(<div className=" col-span-1 rounded-lg bg-gray-700 p-3 m-3 flex " key={index+act.value}>
          <div className="bg-gray-800 mr-3 rounded-full h-8 w-8 flex items-center justify-center text-center text-lg" >{act.value}</div>
          <span>{act.name}</span>
        </div>))}
        </div></div>)}
    </div>
