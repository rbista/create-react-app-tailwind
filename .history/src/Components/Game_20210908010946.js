...
const Game = (props) => {
  ...
  const { gameDuration,setGameDuration, alphaRules, timerSpeed,setTimerSpeed, fontSize, setFontSize, fontCapital,setFontCapital, gameStatus, setGameStatus, defaultData } = props;
  ...
  useEffect(() => {
    if(!gameStatus){setShowTimer(false);
  ...
  return (
    <div className=" flex flex-col flex-wrap justify-center content-center h-full">
      {!showTimer && !showContent && (
        <Settings defaultData={defaultData} gameDuration={gameDuration} setGameDuration={setGameDuration} timerSpeed={timerSpeed} setTimerSpeed={setTimerSpeed} fontCapital={fontCapital} setFontCapital={setFontCapital} fontSize={fontSize} setFontSize={setFontSize} gameStatus={gameStatus} setNoOfChars={setNoOfChars} />
      )}
      {showContent && (
